...
async def timer_checkup_markup(_, CallbackQuery):
    ...
    if await is_active_chat(CallbackQuery.message.chat.id):
        ...
        return await CallbackQuery.answer(f"Musiqi oynamır.", show_alert=True)
    else:
        return await CallbackQuery.answer(
...
@app.on_message(filters.command("Növbə"))
async def activevc(_, message: Message):
    global get_queue
    if await is_active_chat(message.chat.id):
        mystic = await message.reply_text("Zəhmət olmasa gözləyin... Növbə əldə edilir...")
        dur_left = db_mem[message.chat.id]["Sol"]
        duration_min = db_mem[message.chat.id]["Ümumi"]
        got_queue = get_queue.get(message.chat.id)
        if not got_queue:
            await mystic.edit("Növbədə heç bir şey yoxdur")
        fetched = []
        ...
        msg = "**Növbəyə qoyulanların siyahısı**\n\n"
        msg += "**Hal-hazırda oynayır:**"
        msg += "\n▶️" + current_playing[:30]
        msg += f"\n   ╚Tərəfindən:- {user_name}"
        msg += f"\n   ╚Müddət:- Xatırlatma `{dur_left}`çıxmaq `{duration_min}` Dəq."
        fetched.pop(0)
        if fetched:
            msg += "\n\n"
            msg += "**Növbəti:**"
        ...
        await message.reply_text("Növbədə heç nə yoxdur")
...
async def basffy(_, message):
    ...
    try:
        ...
        return await message.reply_text(
            "Öncədən botu qrupa əlavə edin.! Bot üçün naməlum söhbət "
        )
    _assistant = await get_assistant(chat_id, "assistant")
    if not _assistant:
        return await message.reply_text(
            "Öncədən saxladığınız Assistent tapılmadı.\n\nSiz Assistenti qrup daxilində /play vasitəsilə ilə endirə bilərsiniz {Chat}"
        )
    else:
...
async def baaaf(_, message):
    if len(message.command) != 2:
        await message.reply_text(
            "**İstifadəsi :**\n/leavebot [Qrupun adı və qrupun id]"
        )
        return
    chat = message.text.split(None, 2)[1]
    try:
        ...
        await message.reply_text("Uğursuz oldu\n**Mümkün səbəb ola bilər**:{e}")
        print(e)
        return
    await message.reply_text("Bot qrupu uğurla tərk etdi!")
...
async def baujaf(_, message):
    ...
    try:
        ...
        return await message.reply_text(
            "Əvvəlcə bu Söhbətə Botu əlavə edin.. Bot üçün naməlum söhbət"
        )
    _assistant = await get_assistant(chat, "assistant")
    if not _assistant:
        return await message.reply_text(
            "Öncədən saxladığınız Assistent tapılmadı.\n\nSiz Assistent vasitəsilə quraşdıra bilərsiniz /play ad {Chat}'qrup" 
        )
    else:
        ran_ass = _assistant["saveassistant"]
    ASS_ID, ASS_NAME, ASS_USERNAME, ASS_ACC = await get_assistant_details(
